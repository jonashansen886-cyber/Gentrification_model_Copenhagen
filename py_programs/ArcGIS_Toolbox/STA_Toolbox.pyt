...
class STAAnalysisConfig:
    ...
    def __init__(self, config_file="sta_config.json"):
        ...
        self.clusters_feature = config.get(
            "clusters_feature", "clusters_hovedstad_clean"
        )
        self.csv_folder = config.get("csv_folder", "data/raw/long_format_csv")
    ...
    def _get_default_config():
        ...
        return {
            "clusters_feature": "clusters_hovedstad_clean",
            "csv_folder": "data/raw/long_format_csv",
...
class STAModelBuilder:
    ...
    def __init__(self, config):
        ...
        if config.scratch_workspace and config.workspace:
            self.env_manager = arcpy.EnvManager(
                scratchWorkspace=config.scratch_workspace, workspace=config.workspace
            )
        else:
    ...
    def get_csv_files(self, csv_folder):
        ...
        if not os.path.isabs(csv_folder):
            ...
            csv_folder = os.path.join(repo_root, csv_folder)
    ...
    def process_csv_file(self, csv_file, output_dirs):
        ...
        try:
            ...
            csv_table = os.path.join(output_dirs["temp_gdb"], f"Table_{variable_name}")

            # Use TableToTable with geodatabase path
            arcpy.conversion.TableToTable(
                csv_path, output_dirs["temp_gdb"], f"Table_{variable_name}"
            )
            arcpy.AddMessage(f"✓ Table created: {csv_table}")

            space_time_cube = os.path.join(
                output_dirs["space_time_cube"], f"SpaceTC_{variable_name}.nc"
            )
            hotspot_output = os.path.join(
                output_dirs["emerging_hotspot"], f"EHA_{variable_name}"
            )
            # Use different names for shapefile and DBF to avoid conflict
            tsc_shp = os.path.join(
                output_dirs["time_series_cluster"], f"TSC_{variable_name}"
            )
            tsc_dbf = os.path.join(
                output_dirs["time_series_cluster"], f"TSC_{variable_name}_chart"
            )
