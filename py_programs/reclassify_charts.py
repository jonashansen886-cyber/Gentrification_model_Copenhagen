...
def extract_clusters(html_path):
    try:
        ...
        matches = re.findall(
            r"\"name\":\"Cluster ([0-9]+)\".*?\"y\":{\"dtype\":\"f8\",\"bdata\":\"(.*?)\"}",
            content,
        )
        clusters = []
...
def main():
    ...
    files = sorted(
        [
            ...
            if f.endswith(".html") and "counts" not in f and "qol" not in f
        ]
    )
    output_lines = []
    ...
    for filename in files:
        ...
        for c in clusters:
            ...
            nm = (
                (c["mean"] - mean_min) / (mean_max - mean_min)
                if mean_max != mean_min
                else 0.5
            )
            ns = (
                (c["slope"] - slope_min) / (slope_max - slope_min)
        ...
        cluster_str = ", ".join(
            [f"cluster {cid + 1} == {mapping[cid]}" for cid in sorted(mapping.keys())]
        )
